...
class EM_Limmud_Paid {
    ...
    public static function init_actions() {
        ...
        if (!empty($_REQUEST['action']) && ($_REQUEST['action']  == 'paid-callback-status') && !empty($_REQUEST['transaction_id'])) {
            ...
            if (substr($transaction_id_values[0], 0, 6) == 'LIMMUD') {
                ...
                $sale_payment_method = isset($_REQUEST['sale_payment_method']) ? $_REQUEST['sale_payment_method'] : '';
                self::sale_callback($booking_id, $_REQUEST['notify_type'], $transaction_note, $_REQUEST['payme_sale_code'], $_REQUEST['price'], $_REQUEST['currency'], $_REQUEST['sale_created'], $sale_payment_method);
                die();
            }
        }
    }
    ...
    */
    ...
    public static function generate_url($booking_id, $payment_type, $language, $transaction_sum)
    {
        ...
        $redirect_url = get_post_permalink(get_option('dbem_paid_redirect_page')) . '&booking_id=' . $EM_Booking->booking_id . '&secret=' . EM_Limmud_Booking::get_secret($EM_Booking, 'payment_redirect');
    ...
    public static function sale_callback($booking_id, $notify_type, $transaction_id, $payme_sale_code, $amount, $currency, $timestamp, $sale_payment_method)
    {
        $result = "failed";
        ...
        self::log('    sale_payment_method=' . $sale_payment_method, 'debug');

        $EM_Booking = em_get_booking($booking_id);
        if (!empty($EM_Booking->booking_id)) {
            $amount = $amount / 100;
            if (($sale_payment_method != 'bit') || ($notify_type != 'sale-failure')) {
                // do not record failed Bit transactions as these happen too often
                EM_Limmud_Misc::record_transaction($EM_Booking->booking_id, 'paid', $amount, $currency, $date, $uid, $status_code, $approval_number);
            }
